...
# Header
        ...
        if ndim > 0:
            if (ndim == 1):
                if 'unit' in iter(uf_d['grid']['X'].keys()):
                ...
                for coord in xyz:
                    if coord in iter(uf_d['grid'].keys()):
                        if 'unit' in iter(uf_d['grid'][coord].keys()):
                            ufs += '%-20s%-10s;-INDEPENDENT VARIABLE LABEL: ' \
            ...
            if 'unit' in iter(uf_d['data'].keys()):
...
# ... F
        if self.dim == 1:
            ftmp = np.zeros(self.n['X0'])
        elif self.dim == 2:
            ftmp = np.zeros(self.n['X0']*self.n['X1'])  
        elif self.dim == 3:
            ftmp = np.zeros(self.n['X0']*self.n['X1']*self.n['X2'])
        ...
        if self.dim == 1:
            ...
            self.fvalues = ftmp.reshape(self.n['X1'], self.n['X0']).T

        elif self.dim == 3:
            self.fvalues = ftmp.reshape(self.n['X2'], self.n['X1'], self.n['X0']).T
